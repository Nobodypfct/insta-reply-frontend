...
## Карточки шаблонов — статус-полоска (сделано); аналитика/дублирование (TODO)

По референсам конкурентов (ManyChat/ChatPlace, скриншоты в переписке)
пересмотрели карточку шаблона в `app/dashboard/accounts/[id]/page.tsx`.

**Сделано — статус вкл/выкл теперь НЕ `Badge`.** У самого Astryx в доках
`Badge` явный совет не вешать success-бейдж на КАЖДУЮ карточку одинаково
("reserve badges for the exceptional ones" — иначе он превращается в шум,
не в информацию), а старый инлайновый `ActiveStatusBadge` именно так и
делал. Заменено на цветную полоску слева (`border-l-4`,
`border-l-success`/`border-l-border-strong`) + приглушение всей карточки
(`opacity-60`), когда шаблон выключен — считывается периферийным зрением
при скролле списка, выключенные шаблоны визуально "отступают", не спорят
за внимание с активными. `ActiveStatusBadge` по-прежнему используется на
`/dashboard/accounts` для `webhook_enabled` — тут его не убирали, трогали
только карточку шаблона.
