...
**Превью телефона на объединённом шаге 0** — весь шаг целиком показывает
"Комментарии" (`PreviewStep 1`: шторка с примером совпадающего
комментария), не "Пост". Обоснование: пикер поста меняет превью-миниатюру
ЗА шторкой (она её не закрывает целиком), так что видно и выбранный пост,
и как выглядит совпадающий комментарий — одновременно. `PreviewStep 0`
("Пост", голый пост-экран без шторки) технически всё ещё существует в
`PhonePreview.tsx` (не удалён — компонент его по-прежнему умеет
рендерить), но с тех пор, как выбор поста объединили со слово-триггером,
`TemplateWizard` больше никогда его не запрашивает — недостижим с этой
стороны.

**Табы под превью — было 3 ("Пост"/"Комментарии"/"Директ"), стало 2**
("Комментарии"/"Директ") — "Пост" убран как отдельный таб по той же
причине: отдельного шага формы под голый пост-экран больше нет. Табы
рендерятся как явные пары `{label, previewStep}` (не голый индекс тaба,
как раньше) — `onTabClick` отдаёт сразу `PreviewStep` (1 или 2), а
`TemplateWizard.handlePreviewTabClick()` сам мапит его на нужный
`WizardStep` (`previewStep === 1 ? 0 : 2`).

Логика навигации по табам (в `TemplateWizard.tsx`) не изменилась по духу:
...
- таб "Комментарии" визуально накрывает ДВА шага формы (0 — пост+слово,
  1 — ответ на комментарий) — ведёт в НАЧАЛО этой группы (шаг 0), не в
  середину
