...
**Отложено (осознанно, не забыть)**: глобальный 401-interceptor
(`shared/api/client.ts` сейчас вообще не шлёт `Authorization` — нечего
перехватывать) и связанная с ним UX-метка "сессия истекла" на `/login` —
отдельная задача, требует сначала прикрутить Supabase JWT к `apiClient` +
